...
class CollisionSolver {
  ...
  //================ Constructor ================//
  CollisionSolver(FlexibleSheet filament_, Window view_) { // single Sheet
  ...
  CollisionSolver(FlexibleSheet [] filament_, Window view_) { // multiple sheets
  ...
  void ResolveBoundary() {
    ...
    if (SBP>0) {
      ...
      for (int j = 0; j<SBP; j++) {
        ...
        myP.UpdatePosition(myP.position.x, myView.y.inE-myP.thick);
        myP.UpdateVelocity(myP.velocity.x, -myP.velocity.y);
      }
      SBoundPointCol = new ArrayList<ControlPoint>();
    }
    ...
    if (EBP>0) {
      ...
      for (int j = 0; j<EBP; j++) {
        ...
        myP.UpdatePosition(myView.x.inE-myP.thick, myP.position.y);
        myP.UpdateVelocity(-myP.velocity.x, myP.velocity.y);
      }
      EBoundPointCol = new ArrayList<ControlPoint>();
    }
