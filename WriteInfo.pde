class WriteInfo {
  ...
  WriteInfo( FlexibleSheet fs ) {
    ...
    for (int i=0; i<Ncp; i++) { myCPoints[0].add(fs.cpoints[i]); }
    InitPositionOut(); posFlag = true;
    InitVelocityOut(); velFlag = true;
    InitForceOut(); forFlag = true;
    
    for (int i=0; i<Nsg; i++) { mySprings[0].add(fs.springs[i]); }
    InitEnergyOut();  eneFlag = true;
  ...
  WriteInfo(FlexibleSheet [] fs) {
    ...
    for ( int j=0; j<Nfs; j++ ) {
      ...
      for (int i=0; i<fs[j].numOfpoints; i++) { myCPoints[j].add(fs[j].cpoints[i]); }
        
      InitPositionOut( j ); posFlag = true;
      InitVelocityOut( j ); velFlag = true;
      InitForceOut( j ); forFlag = true;
    
      for (int i=0; i<fs[j].numOfsprings; i++) { mySprings[j].add(fs[j].springs[i]); }
  ...
  void InfoCPoints( float t, int d ) {
  ...
  void InfoCPoints( float t ) { InfoCPoints( t, 0 ); }
  ...
  void InfoSprings( float t ) { InfoSprings(t, 0 ); }
